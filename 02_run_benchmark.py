...
def choose_model(saved_model: str) -> str:
    ...
    if not model:
        raise ValueError("A model ID is required. Run 01_configure_benchmark.py or provide --model.")
    return model
...
def main() -> None:
    ...
    # No flags means zero-prompt execution: every choice comes from 01_configure_benchmark.py.
    ...
    if workflow == "dashboard":
        ...
        if not model:
            parser.error("No saved model ID. Run `python3 01_configure_benchmark.py --edit` or provide --model.")
