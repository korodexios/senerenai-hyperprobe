...
Stage 1 uses **33 deliberate combinations**, not an opaque random sweep. It tests one shared baseline, then runs five non-baseline levels for each of `temperature`, `min_p`, `top_p`, and `repetition_penalty`, changing only one parameter at a time. It then adds twelve targeted interaction rows: four `temperature × top_p` corners, four `min_p × top_p` corners, and four `temperature × repetition_penalty` corners. Its purpose is to establish credible parameter influence and useful value ranges before refinement, not to claim an exact optimum. Two samples per prompt and combination are used by default because a single stochastic completion can produce a misleading boundary; use three samples for visibly noisy models or high-confidence production tuning.

The handoff records the baseline, the ranking of every tested main-effect value, an effect span for each parameter, selected values for refinement, and targeted-interaction evidence. Failed API calls remain in JSONL diagnostics but are excluded from ranking evidence. Records generated by this design are tagged `hybrid_v4`; the dashboard displays them separately from older `legacy` search records, even when they use the same model, so their rankings are never silently mixed.
...
For a profile with three quick Stage 1 prompts and two Stage 1 samples, the default full workflow uses at most **198 Stage 1 calls**, **15 Stage 2 calls** (three prompts × five targeted candidates × one sample), and **52 Stage 3 calls** (two holdout prompts × two Stage 2 candidates × thirteen local combinations × one sample), for a practical total of about **265 calls**. The previous v1.8 design used 350 calls for the same example, so the new design saves **85 calls** while retaining the richer interaction and holdout tests. This is the intended funnel: broad evidence first, narrow interaction decision second, and complex local validation last. Increase Stage 1 samples to three only when the model is visibly noisy or the preset will be used in a high-stakes repeated workflow.

The exact count depends on the selected profile’s prompt bank. The general formulas are:

```text
Stage 1 = 33 combinations × Stage 1 prompts × Stage 1 samples
