...
def show_summary(settings: dict[str, Any]) -> None:
    ...
    print(f"  Model:        {settings.get('model') or '(choose in 02_run_sampler_benchmark.py)'}")
...
def interactive_setup(*, force_edit: bool = False) -> None:
    ...
    print("=" * 72)
    print("Senerenai-HyperProbe 01_configure_sampler_benchmark.py — persistent local setup")
    ...
    if existing and not force_edit:
        ...
        if choice != "edit":
            print("Settings unchanged. Run `python3 02_run_sampler_benchmark.py` when you are ready to benchmark.")
            return
    ...
    settings["backend_label"] = ask_text(
        "backend_label",
        "Server/backend label (recommended)",
        str(settings.get("backend_label", "")),
        "A human-readable implementation and version, for example llama.cpp bXXXX or vLLM 0.X. This is saved in result provenance; it does not affect model calls.",
    )
    ...
    print("Next: run `python3 02_run_sampler_benchmark.py` to select profiles and launch a stage or the full pipeline.")
...
def main() -> None:
    ...
    if args.reset:
        ...
        print(f"Reset local settings at {SETTINGS_PATH}. Run `python3 01_configure_sampler_benchmark.py --edit` to configure them.")
        return
