...
def main() -> None:
    ...
    if not model:
        raise SystemExit("No saved model ID. Run `python3 01_setup.py --edit` first.")
    ...
    if "niah" in settings["enabled_modes"]:
        ...
        if not corpus:
            raise SystemExit("NIAH is enabled but no corpus path is saved. Run `python3 03_probe_setup.py`.")
